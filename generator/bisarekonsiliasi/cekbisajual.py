"""Reconcile a list of invoices against the BisaJual ledger to surface entry bugs.

This is a read-only audit aimed at one question: **how was each order entered into
BisaJual, compared to the real money it actually made?** For every invoice it pairs

  * the booked Omzet in the itbisa-shop-report-bot ``*BisaJual*.xlsx`` ledger
    (non-void ``Omzet Barang (Rp)`` across every ``BisaJual*`` sheet, with the void
    amount kept separately), with
  * the real money received in ``BisaSaldo`` (the full net of every line for the
    invoice) and the ``BisaFee`` settlement (``Total Penghasilan`` + the refund/fee
    ``Kerugian``),

then classifies the BisaJual entry:

  BUG: entry hilang          - money came in, but the order is not in BisaJual
  BUG: harusnya Void         - Omzet booked, but no money (a return left un-voided)
  BUG: salah Void            - voided in BisaJual, yet money was received
  BUG: omzet != uang         - booked Omzet doesn't match the money received
  OK : cocok / retur / void  - entry already matches reality

Run it with ``python main.py --cek-bisajual`` (defaults to the built-in list) or
``--cek-bisajual --invoices my_list.txt`` for any other set, and point
``--bisajual-dir`` at the bot repo's ``data/``. It writes
``reports/shopee/Cek BisaJual Shopee.xlsx``.
...
from bisarekonsiliasi.generic import _saldo_files

# Bot BisaJual ledger column (hand-kept, with a Void flag).
_OMZET = 'Omzet\nBarang\n(Rp)'
...
def _bisajual_detail(invoices, bisajual_dir=None):
    """Invoice -> {present, omzet (non-void), void_omzet} from the bot BisaJual files.

    Looks across every ``BisaJual*`` sheet (so a Shopee order booked under e.g.
    ``BisaJualCoD`` still counts as present).
    """
    inv_set = set(invoices)
    files = []
    for folder in [get_data_dir()] + ([bisajual_dir] if bisajual_dir else []):
    ...
    for path in sorted(set(files)):
        ...
        try:
            ...
            logging.warning("Gagal baca BisaJual %s: %s", path, err)
            continue
        for sheet in (s for s in xls.sheet_names if s.startswith('BisaJual')):
            df = xls.parse(sheet)
...
def _saldo_net(invoices):
    """Invoice -> full BisaSaldo net (every Shopee saldo line for that invoice)."""
    inv_set = set(invoices)
    net = {}
    ...
    for path in _saldo_files('BisaSaldo v2 Shopee'):
    ...
    for path in _saldo_files('BisaSaldo v3 Shopee'):
...
def _fee_detail(invoices):
    """Invoice -> {tp (Total Penghasilan), ker (refund+fees as Kerugian)} from BisaFee."""
    inv_set = set(invoices)
    rows = []
...
def _verdict(jual, real, fee):
    """Classify how the BisaJual entry compares to the real money."""
    ...
    if not present:
        return 'BUG: entry hilang (ada uang, tidak di BisaJual)' if has_money \
            else 'OK: retur, tidak dibukukan'
    if omzet > 0 and money <= 0:
        return 'BUG: omzet di BisaJual tapi tidak ada uang -> Void'
...
def reconcile_invoices(invoices=None, bisajual_dir=None):
    """Reconcile the given invoices against BisaJual + BisaSaldo + BisaFee.

    Writes reports/shopee/Cek BisaJual Shopee.xlsx and returns the DataFrame.
    """
    invoices = list(invoices) if invoices else list(DEFAULT_INVOICES)
    jual = _bisajual_detail(invoices, bisajual_dir)
    ...
    for inv in invoices:
        ...
        rows.append({
            'Invoice': inv,
            'Di BisaJual?': 'Ya' if j else 'Tidak',
            'Omzet (BisaJual)': round(j['omzet']) if j else 0,
            'Omzet Void': round(j['void_omzet']) if j else 0,
            'Penghasilan (BisaSaldo)': '' if real is None else round(real),
    ...
    logging.info("Cek BisaJual: %d invoice, %d BUG ditemukan", len(df), bugs)
...
def _write(df):
    ...
    path = os.path.join(folder, 'Cek BisaJual Shopee.xlsx')
    with pd.ExcelWriter(path, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='Cek BisaJual', index=False)
        sheet = writer.book['Cek BisaJual']
