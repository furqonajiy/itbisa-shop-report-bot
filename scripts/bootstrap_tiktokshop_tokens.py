...
NOTE: This bot uses the SAME TikTok app secret as itbisa-tiktokshop-order-bot
and itbisa-shop-stock-bot, but its token chain is INDEPENDENT. Re-authorize
the shop fresh for this repo.
"""
...
def main():
    ...
    print("=" * 60)
    print("TikTok Shop Tokens Bootstrap — itbisa-shop-report-bot")
    print("=" * 60)
    ...
    print("independent from itbisa-tiktokshop-order-bot and itbisa-shop-stock-bot.")
    print()
    ...
    access_expires_at  = datetime.fromtimestamp(int(access_token_expire_in),  tz=timezone.utc).isoformat()
    refresh_expires_at = datetime.fromtimestamp(int(refresh_token_expire_in), tz=timezone.utc).isoformat()
    ...
    print(f"  access_token_expires:   {access_expires_at}")
    print(f"  refresh_token_expires:  {refresh_expires_at}")
    print("\nYou can now run the bot normally. The bot will auto-refresh")
    print("the access_token when needed. The refresh_token is very long-lived.")
