...
NOTE: This bot uses the SAME Shopee partner key as itbisa-shopee-order-bot
and itbisa-shop-stock-bot, but its token chain is INDEPENDENT. You should
re-authorize the shop fresh for this repo so it receives its own access_token
and refresh_token pair that does not collide with the order bot's rotation.
"""
...
def main():
    ...
    print("=" * 60)
    print("Shopee Tokens Bootstrap — itbisa-shop-report-bot")
    print("=" * 60)
    ...
    print()
    print("This bot maintains its own token chain on its own bot-state branch,")
    print("independent from itbisa-shopee-order-bot and itbisa-shop-stock-bot.")
    print("Re-authorize the shop fresh so the new repo gets an independent")
    print("token pair that won't collide with the other bots' rotation.")
    print()
    ...
    # STEP 5: Extract tokens and compute the absolute expiry timestamp.
    ...
    print("the access_token when needed. You only need to run this script")
    print("again when the refresh_token expires (~30 days inactivity).")
