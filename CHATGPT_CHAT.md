...
- `laporan/` — co-located **Laporan generator** (importable `laporan` package, run in-process via `python main.py --laporan` or `python -m laporan`): raw exports → `Laporan` workbooks, delivered as `Jual`/`Remit`(=the combined `Final`)/`Bonus` (builds `Invoice`/`Remit`/`Final` internally, then collapses). Readers tolerate evolving exports (TikTok `Fee` EN/Indonesia columns, broken `.xlsx` dims, Shopee `Fee` floating `Adjustment` header). Its `Jual` feeds this bot's `Jual` ledger through a **manual Google Sheets step** — separate stages, not auto-chained.
...
- **Sisa stok = current-workbook ledger** matching Google Sheets `RekapBarang`: per (SKU, gudang) `arrived beli − nonvoid jual + ketemu − hilang ± pindah`, from the latest stok/jual file by filename. Migrasi = opening balance (kept). Negative gudang floors to 0 (deficit shifted); OVERSOLD (negative total) stays negative + flagged.
