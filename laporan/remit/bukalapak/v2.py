...
def generate_remit(bl_file, df):
    logging.info("Generate Remit Bukalapak from {0} ({1} rows)".format(bl_file, len(df)))
    ...
    # Convert Data Type
    df['Tanggal Remit'] = pd.to_datetime(df['Tanggal Remit'], format='mixed', dayfirst=True).dt.strftime('%Y-%m-%d %H:%M')  # Datetime

    # Aggregate
    df = df.groupby(['Invoice', 'Tanggal Remit']).sum().sort_values('Invoice')

    # Export to Existing WorkBook
    path = bl_file.replace('Saldo', 'Laporan').replace('csv', 'xlsx')
    path = build_report_path(path)
    remit_to_excel(df, path, 'Remit Bukalapak')
