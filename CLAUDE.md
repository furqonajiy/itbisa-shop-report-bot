...
> **Source of truth for the Claude surfaces** — read by **Claude Code** and pasted into the **Claude Chat** project instructions (Claude has no tight char cap, so it gets the full detail). `AGENTS.md` (ChatGPT Codex) points here; `PROJECT_INSTRUCTIONS.md` is the ≤8000-char condensed copy for **ChatGPT Chat** only.
...
- **AI-instruction files**: `CLAUDE.md` (this file) = Claude Code **+ Claude Chat** · `AGENTS.md` = ChatGPT Codex (points here) · `PROJECT_INSTRUCTIONS.md` = the ≤8000-char condensed brief for **ChatGPT Chat** only (ChatGPT's limit). Keep `PROJECT_INSTRUCTIONS.md` in sync with this file's substance but update it **only when explicitly asked**.
